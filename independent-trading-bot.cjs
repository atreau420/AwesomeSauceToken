...
const CONFIG = {
    ...
    // MAXIMUM PROFIT Trading Configuration - NOW TRADES ANY TOKEN YOU OWN!
    TRADE_AMOUNT: 0.001, // Micro trades for any profit
    MIN_PROFIT_THRESHOLD: 0.000001, // Accept ANY profit (even fractions of pennies!)
    MAX_SLIPPAGE: 0.01, // Higher slippage tolerance for micro trades
    MIN_TRADE_SIZE: 0.0001, // Minimum trade size (very small)

    // ANY TOKEN TRADING - Bot will scan and trade whatever you own
    SCAN_ALL_TOKENS: true, // Check all tokens in your wallet
    TRADE_ANY_TOKEN: true, // Trade any token with balance > 0
    MICRO_TRADING: true, // Enable micro-trading for tiny profits
...
class IndependentTradingBot {
    ...
    async initialize() {
        try {
            ...
            try {
                ...
                ethBalance = this.web3.utils.fromWei(balance, 'ether');
                console.log(`💰 Wallet Balance: ${ethBalance} ETH`);
            } catch (balanceError) {
                console.log(`⚠️ Balance check failed: ${balanceError.message}`);
                console.log('🔄 Continuing with balance check disabled...');
                ethBalance = '1.0'; // Assume sufficient balance for now
            }

            if (parseFloat(ethBalance) < CONFIG.TRADE_AMOUNT * 2) {
                console.log(`⚠️ Low balance warning: ${ethBalance} ETH (recommended: ${CONFIG.TRADE_AMOUNT * 2} ETH)`);
                console.log('🔄 Starting with reduced trade amount...');
            }

            // 🔍 SCAN ALL TOKENS IN WALLET
            ...
            this.userTokens.forEach(token => {
                console.log(`  💎 ${token.symbol}: ${token.balance} (${token.usdValue ? '$' + token.usdValue : 'N/A'})`);
            });
    ...
    async updateAllMarketData() {
        // Use tokens found in user's wallet instead of hardcoded pairs
        if (this.userTokens && this.userTokens.length > 0) {
            for (const token of this.userTokens) {
                if (token.symbol !== 'ETH') { // Skip ETH for now, focus on ERC-20 tokens
                    try {
                        // Try to get price data for this token vs ETH or USDT
                        const marketData = await this.getTokenMarketData(token);
                        if (marketData) {
                            this.updatePriceHistory(token.symbol + 'USDT', marketData);
                        }
                    } catch (error) {
            ...
            // Fallback to original pairs if no tokens found
    ...
    async executeArbitrageStrategy() {
        ...
        // Check for price differences across DEXes with ANY tokens user owns
        const opportunities = [];

        // Use tokens found in user's wallet
        const tokensToTrade = this.userTokens && this.userTokens.length > 0 ? this.userTokens : CONFIG.TRADING_PAIRS;

        for (const token of tokensToTrade) {
            if (token.symbol === 'ETH') continue; // Skip ETH for now

            try {
                const prices = await this.getTokenPricesAcrossDEXes(token);

                if (prices.length >= 2) {
                    const sortedPrices = prices.sort((a, b) => a.price - b.price);
                    const spread = (sortedPrices[sortedPrices.length - 1].price - sortedPrices[0].price) / sortedPrices[0].price;

                    // Lower minimum spread for micro profits
                    if (spread >= 0.001) { // 0.1% minimum spread for ANY profit
                        opportunities.push({
                            token: token.symbol,
                ...
                console.log(`⚠️ Could not check arbitrage for ${token.symbol}: ${error.message}`);
            }
        }
            }
        }
        ...
        if (opportunities.length > 0) {
            ...
            console.log(`🔄 ARBITRAGE: ${bestOpp.token} - Spread: ${(bestOpp.spread * 100).toFixed(2)}%`);
            return await this.executeTokenArbitrageTrade(bestOpp);
        }
    ...
    async executeMomentumStrategy() {
        ...
        for (const token of tokensToTrade) {
            ...
            try {
                const prices = this.priceHistory[token.symbol + 'USDT'];
                if (!prices || prices.length < 10) continue;

                const momentum = this.calculateMomentum(prices.slice(-10));
                if (Math.abs(momentum) >= CONFIG.STRATEGIES.MOMENTUM.threshold) {
                    const direction = momentum > 0 ? 'BUY' : 'SELL';
                    console.log(`📈 MOMENTUM: ${token.symbol} - ${direction} signal (${momentum.toFixed(4)})`);
                    return await this.executeTokenMomentumTrade(token, direction);
                }
            } catch (error) {
                console.log(`⚠️ Momentum check failed for ${token.symbol}: ${error.message}`);
            }
        }
    ...
    // 🔍 SCAN ALL TOKENS IN USER'S WALLET
    async scanWalletTokens() {
        ...
        try {
            ...
            if (parseFloat(ethBalanceFormatted) > 0) {
                tokens.push({
                    symbol: 'ETH',
                    address: '0x0000000000000000000000000000000000000000', // ETH has no contract address
                    balance: ethBalanceFormatted,
                    decimals: 18,
                    usdValue: null // Will be calculated later
                });
            }
        } catch (error) {
        ...
        const commonTokens = [
            ...
            { symbol: 'GNT', address: '0xa74476443119A942dE498590Fe1f2454d7D4aC0d', decimals: 18 }
        ];
    ...
    // 📊 GET MARKET DATA FOR ANY TOKEN
    async getTokenMarketData(token) {
        try {
            // Use CoinGecko API for price data
            const response = await fetch(`https://api.coingecko.com/api/v3/simple/token_price/ethereum?contract_addresses=${token.address}&vs_currencies=usd&include_24hr_change=true`);
            ...
            console.log(`⚠️ Could not fetch price for ${token.symbol}: ${error.message}`);
        }
